...
#  linear layer self.llm_proj 是一个用于 特征空间映射 的全连接线性层，其主要作用是将 Q-former 的输出特征转化为适合语言模型（LLM）输入的格式
    ...
    def encode_image(self, image):
        with (self.maybe_autocast() if self.enable_autocast else contextlib.nullcontext()):
            ...
            image_embeds = self.ln_vision(image_embeds)                                        # 图像特征的归一化层，可能是 LayerNorm，用于平衡特征分布
